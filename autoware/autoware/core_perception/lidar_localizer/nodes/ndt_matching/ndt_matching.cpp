...
static void map_callback(const sensor_msgs::PointCloud2::ConstPtr& input)
{
  ...
  if (points_map_num != input->width)
  {
    ...
    if (_use_local_transform == true)
    {
      ...
      {
        ROS_ERROR("[%s] %s", __APP_DATA__, ex.what());
      }
...
int main(int argc, char** argv)
{
  ...
  {
    ROS_WARN("[%s] Query base_link to primary lidar frame through TF tree failed: %s", __APP_DATA__, ex.what());
    received_tf = false;
  }
  ...
  if (!received_tf)
  {
    ...
    {
      ROS_WARN("[%s] Query base_link to primary lidar frame through tf_baselink2primarylidar param failed", __APP_DATA__);
    }
  }
  ...
  if (!received_tf)
  {
    ...
    {
      ROS_WARN("[%s] Query base_link to primary lidar frame through tf_* params failed", __APP_DATA__);
    }
  }

  if (received_tf)
  {
    ROS_INFO("[%s] base_link to primary lidar transform queried successfully", __APP_DATA__);
  }
  else
  {
    ROS_ERROR("[%s] Failed to query base_link to primary lidar transform", __APP_DATA__);
    return 1;
  }
