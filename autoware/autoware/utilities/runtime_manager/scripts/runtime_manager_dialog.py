...
class BarLabel(wx.Panel):
	...
	def OnPaint(self, event):
		...
		if self.IsEnabled():
			...
			rect = wx.Rect(0, 0, round(p), h) if self.hv == wx.HORIZONTAL else wx.Rect(0, round(h-p), w, round(p))
			dc.GradientFillLinear(rect, self.col1, self.col2, self.dir)
			rect = wx.Rect(round(p), 0, round(w-p), h) if self.hv == wx.HORIZONTAL else wx.Rect(0, 0, w, round(h-p))
...
def scaled_bitmap(bm, scale):
	...
	img = img.Scale(round(w * scale), round(h * scale), wx.IMAGE_QUALITY_HIGH)
