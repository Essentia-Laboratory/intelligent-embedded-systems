...
void SaveDepth::depthConverter(int argc, char* argv[]){
  ...
  ros::NodeHandle n;
  ROS_INFO("[%s] subscribe image_topic_name=[%s]", "Depth_Converter", image_topic_name_.c_str());  
  ros::Subscriber image_sub = n.subscribe(image_topic_name_, 1, &SaveDepth::get_image, this);
  ROS_INFO("[%s] subscribe pc2_topic_name=[%s]", "Depth_Converter", pc2_topic_name_.c_str());  
